...
<input class="form__input form__input--tel" type="tel" name="tel" id="paymentTel" required placeholder="Номер телефона" />
<input class="form__input form__input--name" type="text" name="name" id="paymentName" required placeholder="Ваше ФИО" />
<input class="form__input form__input--email" type="email" name="email" id="paymentEmail" required placeholder="Электронная почта" />
<input class="form__input form__input--value" type="number" step="0.01" name="amount" id="paymentAmount" required placeholder="Сумма оплаты" />
<button type="submit" class="form__submit payment-form__submit">Оплатить заказ</button>
<span class="form__submit-subtitle">Нажимая кнопку вы даете согласие на <a class="link" href="/privacy/">обработку данных</a></span>
</form>
</div>
<style>{`
...
const response = await fetch('https://autopragmat.ru/api/v1/send-pay/', {
method: 'POST',
body: data,
});
if (!response.ok) {
...
export default function PaymentForm(_props: Record<string, any>): any { }
